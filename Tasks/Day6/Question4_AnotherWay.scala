/*Another way for Question4 implicit conversion*/
...
implicit def studentArrayToList(studentArray: Array[Student]): ListBuffer[Student] = ListBuffer(ArraySeq.unsafeWrapArray(studentArray)*)
...
class StudentOps(val students: ListBuffer[Student]) {
  // Filter students based on a given filter method
  def filterStudents(filterMethod: Student => Boolean): ListBuffer[Student] = {
